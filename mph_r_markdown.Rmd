...
```{r load acs data to avoid that chunk}
load(file = "acs_geo_cats.rdata")
```


```{r add population-weighted centroids}
...
```{r merges to make the big dataset}
...
lime_fluid_swd_horiz_ruca <-   # merge with ruca data
  full_join(lime_fluid_swd_horiz, 
            ruca_block_group, 
            by = c("GEOID" = "block_group_geoid"))

# merge with production well data
lime_fluid_swd_horiz_ruca_prod <-   # merge with ruca data
  full_join(lime_fluid_swd_horiz_ruca, 
            prod_block_group, 
            by = c("GEOID"))

# merge with pop-weighted centroids
lime_fluid_swd_horiz_ruca_prod_cent <- 
  full_join(lime_fluid_swd_horiz_ruca_prod, 
            block_group_centroids_2010_pop_weight_fixed, 
            by = c("GEOID"))

#### make the full dataset ####
# merge everything with acs!  hey-o!
merged_tibbles <- merge(lime_fluid_swd_horiz_ruca_prod_cent,
                                  acs_geo_cats,
                                  by = "GEOID",
                                  all = TRUE)
...
```{r simplify to simple variables}
...
# make populated into a tibble
ks_analysis_populated <- 
  as_tibble(ks_analysis_populated)
ks_analysis_all <- 
  as_tibble(ks_analysis_all)
...
spatial_residuals <- 
  tibble(GEOID = complete_case_dataset$GEOID, 
    residuals = residual_best_aic, 
            longitude = complete_case_dataset$INTPTLON, 
            latitude = complete_case_dataset$INTPTLAT)
