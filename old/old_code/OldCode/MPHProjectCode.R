...
welldata<-read.csv("welldatabase20170824.csv")
View(welldata)
length(unique(welldata$API14))
welldataforjoin<-welldata[c("API14","Surface.Hole.Longitude","Surface.Hole.Latitude")]
View(welldataforjoin)
write.csv(welldataforjoin,"welldataforjoin.csv")

wellACSjoin<-read.csv("welldatajoinedtoACS2015fixed.txt")
ACSdata<-read.csv("ACS2015dataexport.txt")
ACSmetadata<-read.csv("ACS2015metadata.txt")
welldataprejoin<-read.csv("welldatabase20170824.csv")
welldataforjoin<-read.csv("welldataforjoinfixed.csv")
...
fulldata<-merge(x=ACSwithfullwells,y=selectACSdata,by="GEOID",all.x = TRUE)
save(fulldata,file = "fulldata.rdata")
write.csv(fulldata,file="WolfMPHProjectFullDataset.csv")

load(fulldata.rdata)
table(fulldata$Production.Type)
